...
export function deriveShellLayout(layout: LayoutPreferences): ShellLayout {
  ...
  const leftVisible = !(queueCollapsed && queuePanel === 'left') && navPanel !== 'left'
  const rightVisible = !(queueCollapsed && queuePanel === 'right') && navPanel !== 'right'
  ...
  const fromResizablePct = (size: number | undefined, fallback: number): number =>
    size === undefined ? fallback : roundPanelSize((size / 100) * resizableTotal)
  ...
  return {
    ...
    leftVisible,
    rightVisible,
    leftMin: pct(layout.collapsed.left && queuePanel !== 'left' ? MIN_MINI : MIN_LEFT),
    leftMax: pct(layout.collapsed.left && queuePanel !== 'left' ? MINI_PANEL_MAX : MAX_LEFT),
    rightMin: pct(layout.collapsed.right && queuePanel !== 'right' ? MIN_MINI : MIN_RIGHT),
    rightMax: pct(layout.collapsed.right && queuePanel !== 'right' ? MINI_PANEL_MAX : MAX_RIGHT),
