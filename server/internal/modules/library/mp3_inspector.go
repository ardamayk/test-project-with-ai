...
func inspectID3Tag(file *os.File) (id3Tag, error) {
	...
	for _, issue := range InspectionIssues(parseErr) {
		if issue.Field == "artwork" {
			artwork, artworkErr = optionalArtwork(AlbumArtwork{}, inspectionError(INSPECTION_ERROR_INVALID_ARTWORK, "artwork", errors.New(issue.Reason)))
			parseErr = excludeInspectionFields(parseErr, []string{"artwork"})
			break
		}
	}
...
func collectID3Frame(values *id3Values, frame id3Frame, version byte) error {
	...
	if key != "" {
		if (key == "GENRE" || key == "DATE") && len(frame.payload) == ID3_ENCODED_TEXT_OFFSET && frame.payload[ID3_TEXT_ENCODING_OFFSET] <= ID3_TEXT_ENCODING_UTF8 {
			return nil
		}
